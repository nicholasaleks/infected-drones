...
| ID | Product | Sev | CWE | Sink class | Delivery class | Infected FC / supply chain? | Malicious on-bus peripheral? | Compromised companion? | WiFi/UDP inject? | TCP/cloud MITM? | [SiKW00F](https://github.com/nicholasaleks/sikw00f) injection? | SiK full-MITM? | Fix |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| [QGC-01](QGC-01_camera_info_path_traversal_write/) | QGroundControl | 🟠 HIGH | 22/73/170 | CAMERA_INFORMATION → path-traversal write (**zero-click write upon connection**) | push (CAMERA_INFORMATION) + content via cam_definition_uri | ✅ | ✅ most natural (camera) | ✅ | ✅ | ✅ | ✅ trigger is push; name fields inline | ✅ | [branch](https://github.com/nicholasaleks/qgroundcontrol/tree/fix/qgc-01-camera-info-path-traversal) |
| [QGC-02](QGC-02_component_metadata_uri_ssrf/) | QGroundControl | 🔴 **CRITICAL** | 22/73/494/918 | COMPONENT_METADATA uri → arbitrary file write → **zero-click RCE upon connection** | request/response (URI fetch over IP) | ✅ | ✅ emits metadata uri | ✅ | ✅ for trigger | ✅ | ⚠️ push trigger yes; fetch is over IP | ✅ | [branch](https://github.com/nicholasaleks/qgroundcontrol/tree/fix/qgc-02-download-output-path) |
| [QGC-03](QGC-03_ftp_download_write_and_offset/) | QGroundControl | 🟠 HIGH | 22/770 | FTP listing → traversal write + offset disk-fill | handshake (MAVFTP listing/download) | ✅ | ✅ peer answers FTP | ✅ | ⚠️ inject yes, own-handshake harder | ✅ | ⚠️ racy session/seq | ✅ bridge serves listing | [branch](https://github.com/nicholasaleks/qgroundcontrol/tree/fix/qgc-03-ftp-download-filename) |
| [QGC-04](QGC-04_dataflash_bin_parser_oob/) | QGroundControl | 🟡 MED | 191/125 | DataFlash `.bin` parser OOB read + underflow | handshake (log download of malicious .bin) | ✅ | ✅ peer serves DataFlash | ✅ | ⚠️ | ✅ | ⚠️ must serve log chunks | ✅ bridge feeds .bin | [branch](https://github.com/nicholasaleks/qgroundcontrol/tree/fix/qgc-04-dataflash-fmt-validation) |
| [MP-01](MP-01_mavftp_plugin_rce/) | Mission Planner | 🟠 HIGH | 22→94 | MAVFTP traversal → plugin-loader RCE | handshake (MAVFTP req/ack → RCE) | ✅ | ✅ peer answers listing | ✅ full link control | ⚠️ inject easy, but need to own handshake | ✅ rewrite at relay | ⚠️ must win race + match session/seq | ✅ bridge owns FTP convo | [branch](https://github.com/nicholasaleks/MissionPlanner/tree/fix/mp-01-mavftp-filename-traversal) |
| [MP-02](MP-02_gst_pipeline_injection/) | Mission Planner | 🟠 HIGH | 94/78 | `gst://` → `gst_parse_launch` (file r/w + exfil) | push (VIDEO_STREAM_INFORMATION → gst) | ✅ | ✅ camera announces uri | ✅ | ✅ UDP datagram | ✅ | ✅ one-shot stream frame | ✅ trivially | [branch](https://github.com/nicholasaleks/MissionPlanner/tree/fix/mp-02-gst-pipeline-passthrough) |
| [MP-03](MP-03_statustext_process_start/) | Mission Planner | 🟠 HIGH | 74/601 | STATUSTEXT markup → `Process.Start` | push (STATUSTEXT → ShellExecute) | ✅ | ✅ any component emits | ✅ | ✅ | ✅ | ✅ fire-and-forget text | ✅ | [branch](https://github.com/nicholasaleks/MissionPlanner/tree/fix/mp-03-statustext-link-scheme) |
| [MP-04](MP-04_param_total_connect_dos/) | Mission Planner | 🟡 MED | 248/20/1050 | `RALLY_TOTAL`/`FENCE_TOTAL` → unvalidated `int.Parse` + O(n²) on the UI thread (**zero-click on connect**) | push (PARAM_VALUE, auto post-connect) | ✅ | ✅ emits PARAM_VALUE | ✅ | ✅ | ✅ | ⚠️ must land the param in MP's dict | ✅ | [branch](https://github.com/nicholasaleks/MissionPlanner/tree/fix/mp-04-param-total-tryparse) |
| [MAVSDK-01](MAVSDK-01_component_metadata_curl_ssrf/) | MAVSDK | 🟡 MED | 918/749 | component-metadata curl SSRF + `file://` (no protocol allowlist) | request/response (COMPONENT_METADATA.uri fetch over IP) | ✅ | ✅ emits metadata uri | ✅ | ✅ for trigger | ✅ | ⚠️ push trigger yes; fetch over IP | ✅ | none |
| [MAVSDK-02](MAVSDK-02_camera_cam_definition_ssrf_traversal/) | MAVSDK | 🟠 HIGH | 22/73/918 | cam-definition SSRF + `mftp://` traversal → **zero-click destruction of a file outside the sandbox** | push (CAMERA_INFORMATION.cam_definition_uri, auto) | ✅ | ✅ most natural (camera) | ✅ | ✅ | ✅ | ✅ auto-consumed push | ✅ | [branch](https://github.com/nicholasaleks/MAVSDK/tree/fix/mavsdk-02-camdef-traversal) |
